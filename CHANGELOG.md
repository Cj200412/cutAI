...
- Added `NeutralTimelineV1`, a fixed-argument MLT/`melt` capability probe, and an explicit experimental CPU export path with registered-media resolution, deterministic XML, fixed no-shell arguments, progress, user cancellation, timeouts, process-tree termination, and fail-closed feature compatibility checks. Discovered hardware codecs remain unverified candidates; the existing automatic export path (browser first, Remotion compatibility fallback) remains the default.
  新增版本化、基于帧的 `NeutralTimelineV1` 交换边界、仅使用固定参数的 MLT/`melt` 能力探针，以及可显式选择的实验性 CPU 导出路径；该路径包含注册素材解析、确定性 XML、固定无 shell 参数、进度、用户取消、超时、进程树终止与“能力不兼容即拒绝”的保真检查。探针发现的硬件编码器仍只标记为未验证候选；现有自动导出链（浏览器优先、Remotion 兼容回退）仍为默认。
